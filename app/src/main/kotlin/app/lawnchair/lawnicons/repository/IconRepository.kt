                    ...
                    val filtered =
                        it
                            .mapNotNull { candidate ->
                                ...
                                val matchAtWordStart =
                                    indexOfMatch == 0 ||
                                        candidate.name[indexOfMatch - 1] ==
                                        ' '
                                SearchInfo(
                                    iconInfo = candidate,
                            ...
                            .sortedWith(
                                compareBy(
                                    { searchInfo ->
                                        !searchInfo.matchAtWordStart
                                    },
                                    { searchInfo ->
                                        searchInfo.indexOfMatch
                                    },
                                ),
                            )
                            .map { searchInfo -> searchInfo.iconInfo }
                    IconInfoModel(
                        iconCount = it.size,
                        iconInfo = filtered,
                    )
